...
import sys
import json
from figure_caption_extension import FigureCaptionExtension
from wrap_headings_extension import WrapHeadingsExtension
...
def get_packageOPF_XML(md_filenames=[],image_filenames=[],css_filenames=[],description_data=None):
    ...
    for i,md_filename in enumerate(md_filenames):
        ...
        x.setAttribute('href',"s{:05d}-{}.xhtml".format(i,".".join(md_filename.rsplit(".", 1)[:-1])))
        x.setAttribute('media-type',"application/xhtml+xml")
        manifest.appendChild(x)
...
#         toc_xhtml += """<li><a href="s{:05d}-{}.xhtml">{}</a></li>""".format(i,".".join(md_filename.rsplit(".", 1)[:-1]),".".join(md_filename.rsplit(".", 1)[:-1]))
#     toc_xhtml += """</ol>\n</nav>\n</body>\n</html>"""
# 
#     return toc_xhtml

def get_TOC_XML(default_css_filenames, markdown_filenames):
    """Returns the XML data for the TOC.xhtml file with nested hierarchy.
    
    Each chapter name may be prefixed with one or more double spaces ("  ");
    each double space increases the nesting level by one.
    """
    toc_xhtml = '<?xml version="1.0" encoding="UTF-8"?>\n'
    toc_xhtml += '<html xmlns="http://www.w3.org/1999/xhtml" '
    toc_xhtml += 'xmlns:epub="http://www.idpf.org/2007/ops" lang="en">\n'
    toc_xhtml += '<head>\n<meta http-equiv="default-style" '
    toc_xhtml += 'content="text/html; charset=utf-8"/>\n'
    toc_xhtml += '<title>Contents</title>\n'
    for css_filename in default_css_filenames:
        toc_xhtml += '<link rel="stylesheet" href="css/{}" type="text/css"/>\n'.format(css_filename)
    toc_xhtml += '</head>\n<body>\n'
    toc_xhtml += '<nav epub:type="toc" role="doc-toc" id="toc">\n'
    toc_xhtml += '<h2>Contents</h2>\n'
    
    # Open the root ordered list.
    toc_xhtml += '<ol epub:type="list">'
    
    current_level = 0
    for i, md_filename in enumerate(markdown_filenames):
        # Remove file extension and get the raw chapter title.
        chapter_text = ".".join(md_filename.rsplit(".", 1)[:-1])
        # Count how many leading double spaces there are (each double space counts as one level)
        level = 0
        while chapter_text.startswith("  "):
            level += 1
            chapter_text = chapter_text[2:]
        new_level = level

        # Adjust the nested list structure based on the new level.
        if new_level > current_level:
            # Going deeper: open one (or more) nested <ol> tags.
            for _ in range(new_level - current_level):
                toc_xhtml += '<ol>'
        elif new_level < current_level:
            # Going back up: close nested lists until we reach the new level.
            for _ in range(current_level - new_level):
                toc_xhtml += '</li></ol>'
            toc_xhtml += '</li>'
        else:
            # Same level: close the previous list item (if not the very first item).
            if i != 0:
                toc_xhtml += '</li>'

        # Use a trimmed version of the original label (without extra spaces) for the filename.
        link_label = ".".join(md_filename.rsplit(".", 1)[:-1]).strip()
        toc_xhtml += '<li><a href="s{:05d}-{}.xhtml">{}</a>'.format(i, link_label, chapter_text)
        current_level = new_level

    # Close any still-open list items and nested <ol>s.
    toc_xhtml += '</li>'
    for _ in range(current_level):
        toc_xhtml += '</ol></li>'

    # Close the root <ol> and the rest of the document.
    toc_xhtml += '</ol>\n'
    toc_xhtml += '</nav>\n</body>\n</html>'
...
#         toc_ncx += """<navLabel>\n<text>{}</text>\n</navLabel>""".format(".".join(md_filename.rsplit(".", 1)[:-1]))
#         toc_ncx += """<content src="s{:05d}-{}.xhtml"/>""".format(i, ".".join(md_filename.rsplit(".", 1)[:-1]))
#         toc_ncx += """ </navPoint>"""
#     toc_ncx += """</navMap>\n</ncx>"""
# 
#     return toc_ncx
...
def get_TOCNCX_XML(markdown_filenames):
    """Returns the XML data for the TOC.ncx file with a nested hierarchy.
    
    Each chapter title may be prefixed with one or more double spaces ("  ");
    each such pair increases the nesting level by one.
    """
    nav_points = ""
    stack = []  # This will track open <navPoint> elements.
    
    for i, md_filename in enumerate(markdown_filenames):
        # Remove file extension and get the raw chapter title.
        chapter_text = ".".join(md_filename.rsplit(".", 1)[:-1])
        # Count the number of leading double spaces; each pair increases the level.
        level = 0
        while chapter_text.startswith("  "):
            level += 1
            chapter_text = chapter_text[2:]
        # Use the trimmed title also for the file name in the src attribute.
        link_label = chapter_text.strip()
        # In our stack, we want to keep all ancestors open.
        # The desired “depth” (number of open navPoint tags) is level+1.
        desired_depth = level + 1

        # If we have too many open navPoints, close until we reach the desired depth.
        while len(stack) > desired_depth:
            nav_points += "</navPoint>\n"
            stack.pop()
        # If at the desired depth a previous sibling is still open,
        # close it so that the new navPoint becomes its sibling.
        if stack and len(stack) == desired_depth:
            nav_points += "</navPoint>\n"
            stack.pop()

        # Open a new navPoint.
        nav_points += '<navPoint id="navpoint-{}">\n'.format(i)
        nav_points += '  <navLabel>\n    <text>{}</text>\n  </navLabel>\n'.format(chapter_text.strip())
        nav_points += '  <content src="s{:05d}-{}.xhtml"/>\n'.format(i, link_label)
...
def get_chapter_XML(md_filename,css_filenames):
    ...
    html_text = markdown.markdown(markdown_data,
                                  extensions=["codehilite","tables","fenced_code","footnotes", FigureCaptionExtension(), WrapHeadingsExtension()],
...
if __name__ == "__main__":
    ...
    with zipfile.ZipFile(output_path, "w" ) as myZipFile:
        ...
        for i,chapter in enumerate(json_data["chapters"]):
            ...
            myZipFile.writestr("OPS/s{:05d}-{}.xhtml".format(i,".".join(chapter_md_filename.rsplit(".", 1)[:-1])),
